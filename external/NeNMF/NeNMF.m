...
for iter=1:MaxIter,
    ...
    % if verbose || rem(iter,testIter)
    deltaT = GetStopCriterion(STOP_RULE,[W,H],[GradW,GradH]);
    % disp(deltaT);
    % end
    ...
    if verbose,
        ...
        HIS.prjg=[HIS.prjg,deltaT];
        if (verbose==2) && (rem(iter,testIter)==0),
            fprintf('%d:\tstopping criteria = %e,\tobjective value = %f.\n', iter,deltaT/init_delta,HIS.objf(end)+constV);
        end
    end
    
    % Stopping condition
    if (deltaT<=tol*init_delta && iter>=MinIter) || HIS.cpus(end)>=MaxTime,
        break;
    end
end
W=W';
