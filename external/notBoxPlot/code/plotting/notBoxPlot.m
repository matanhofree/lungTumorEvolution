function varargout=notBoxPlot(y,x,jitter,style,prepFunc,statType)
...
if nargin<3
    jitter=0.2; %larger value means greater amplitude jitter
end
...
if nargin<5
    prepFunc = []; %Can also be 'line' or 'sdline'
end
...
if isvector(y) & isvector(x) & length(x)>1
    ...
    for ii=1:length(u)
        ...
        if exist('prepFunc','var') && ~isempty(prepFunc)
            zSel = prepFunc(y(f));
            f = f(zSel);
        end
        
        h(ii)=notBoxPlot(y(f),u(ii),jitter,style,prepFunc);
    end
    ...
    if ~isempty(zGN)
        set(gca,'xticklabel',zGN,'TickLabelInterpreter','none');
    end
...
for ii=1:length(uX)
    ...
    h=[h,myPlotter(x(f),y(:,f),statType)];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function h=myPlotter(X,Y,statType)
               
        pscale=2;
        if statType == 1
            [SEM,SD]=SEM_calc(Y); %Supplied external function
            % SD=nanstd(Y);  %Requires the stats toolbox
            mu=nanmean(Y); %Requires the stats toolbox           
        elseif statType == 2                        
            mu=nanmedian(Y); %Requires the stats toolbox
            SD = mad(Y,1);             % 0.95 CI factor, MAD to std fudge
            SEM =  (SD ./ sqrt(sum(~isnan(Y))))* 1.96 * 1.4826; 
        end
        ...
        cols(1,:)=0;
        if (jitter == 0)
            jitScale=0.3*0.55*2; %To scale the patch by the width of the jitter
        else
            jitScale=jitter*0.55*2; %To scale the patch by the width of the jitter
        end
        ...
        for k=1:length(X)
            ...
            if (jitter > 0)
                ...
                h(k).data=plot(thisX+J, thisY, 'o','color',C, 'markerfacecolor', C+(1-C)*0.65);
                
            end
            
            if strcmp(style,'patchSD')
                h(k).sdPtch=patchMaker(SD(k),[0.6,0.6,1]);
            end
            
            if strgrep(style,'patch') || strcmp(style,'sdline')
                h(k).semPtch=patchMaker(SEM(k),[1,0.6,0.6]);
                h(k).mu=plot([X(k)-jitScale*pscale,X(k)+jitScale*pscale],[mu(k),mu(k)],'-r',...
                    'linewidth',2);
            end
        ...
        function ptch=patchMaker(thisInterval,color)
            ...
            l=mu(k)-thisInterval;
            u=mu(k)+thisInterval;
            ptch=patch([X(k)-jitScale*pscale, X(k)+jitScale*pscale, X(k)+jitScale*pscale, X(k)-jitScale*pscale],...
                [l,l,u,u], 0);
            set(ptch,'edgecolor',color*0.8,'facecolor',color,'facealpha',0.8)
