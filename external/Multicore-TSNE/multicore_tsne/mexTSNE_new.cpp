...
void TSNE<treeT, dist_fn>::run(double* X, int N, int D, double* Y,
               ...
               double *final_error,int doNormalize) {

    if (N - 1 < 3 * perplexity) {
        perplexity = (N - 1) / 3;
        if (verbose)
            mexPrintf("Perplexity too large for the number of data points! Adjusting ...\n");
    }
    ...
    */
    ...
    if (verbose)
        mexPrintf( "Using no_dims = %d, perplexity = %f, and theta = %f\n", no_dims, perplexity, theta);
    ...
    if (dY == NULL || uY == NULL || gains == NULL) { mexPrintf( "Memory allocation failed!\n"); exit(1); }
    ...
    if (doNormalize == 1) {
        ...
        if (verbose)
            mexPrintf( "Normalizing input data\n");
    ...
    if (verbose)
        mexPrintf( "Computing input similarities...\n");
    ...
    */
    ...
    for (int iter = 0; iter < max_iter; iter++) {
        ...
        if (need_eval_error) {
            ...
            if (iter == 0)
                mexPrintf( "Iteration %d: error is %f\n", iter + 1, error);
            else {
                total_time += (float) (end - start);
                mexPrintf( "Iteration %d: error is %f (50 iterations in %4.2f seconds)\n", iter + 1, error, (float) (end - start) );
            }
            start = time(0);
        }
...
void TSNE<treeT, dist_fn>::computeGaussianPerplexity(double* X, int N, int D, int** _row_P, int** _col_P, double** _val_P, double perplexity, int K, int verbose) {
    ...
    if (perplexity > K) mexPrintf( "Perplexity should be lower than K!\n");

    // Allocate the memory we need
    *_row_P = (int*)    malloc((N + 1) * sizeof(int));
    *_col_P = (int*)    calloc(N * K, sizeof(int));
    *_val_P = (double*) calloc(N * K, sizeof(double));
    if (*_row_P == NULL || *_col_P == NULL || *_val_P == NULL) { mexPrintf( "Memory allocation failed!\n"); exit(1); }
...
//             #pragma omp critical
// #endif
//             mexPrintf( " - point %d of %d\n", steps_completed, N);
//         }

        // Matlab is unable to print in openmp paralel blocks
        #ifndef _OPENMP
        ++steps_completed;
...
void TSNE<treeT, dist_fn>::symmetrizeMatrix(int** _row_P, int** _col_P, double** _val_P, int N) {
    ...
    if (row_counts == NULL) { mexPrintf( "Memory allocation failed!\n"); exit(1); }
    for (int n = 0; n < N; n++) {
        for (int i = row_P[n]; i < row_P[n + 1]; i++) {
    ...
    if (sym_row_P == NULL || sym_col_P == NULL || sym_val_P == NULL) { mexPrintf( "Memory allocation failed!\n"); exit(1); }
    ...
    if (offset == NULL) { mexPrintf( "Memory allocation failed!\n"); exit(1); }
    for (int n = 0; n < N; n++) {
...
void TSNE<treeT, dist_fn>::zeroMean(double* X, int N, int D) {
    ...
    if (mean == NULL) { mexPrintf( "Memory allocation failed!\n"); exit(1); }
    for (int n = 0; n < N; n++) {
        for (int d = 0; d < D; d++) {
            mean[d] += X[n * D + d];
        }
...
//     {
//         if (verbose)
//             mexPrintf( "Performing t-SNE using %d cores.\n", NUM_THREADS(num_threads));
//         if (distance == 0) {
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    int no_dims = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[1]))));
    double perplexity = *(mxGetPr(prhs[2]));
    double theta = *(mxGetPr(prhs[3]));
    int num_threads = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[4]))));
    int max_iter = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[5]))));
    
    int random_state = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[6]))));
    int verbose = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[7]))));

    double early_exaggeration = *(mxGetPr(prhs[8]));
    double learning_rate = *(mxGetPr(prhs[9]));

    int distance = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[10]))));
    int n_iter_early_exag = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[11]))));
 

    bool init_from_Y = false;
    int doNormalize = 0;

    plhs[0] = mxCreateDoubleMatrix(N,no_dims,mxREAL);
    plhs[1] = mxCreateDoubleMatrix(1,1,mxREAL);
    
    double* Y = mxGetPr(plhs[0]);
    double* final_error = mxGetPr(plhs[1]);
//    for (int i = 0; i < N * D; i++) {
//        mexPrintf("%d) %f\n", i, X[i]);
//    }
// #ifdef DEBUG_P
// #endif
    ...
    if (verbose) 
    {
        mexPrintf("Performing multicore t-SNE using %d cores.\n", NUM_THREADS(num_threads));
        mexPrintf("N by D: %d x %d\n",N,D);
        mexPrintf("no_dims: %d\n",no_dims);
        mexPrintf("perplexity: %f\n",perplexity);
        mexPrintf("theta: %f\n",theta);
        mexPrintf("num_threads: %d\n",num_threads);
        mexPrintf("max_iter: %d\n",max_iter);
        mexPrintf("random_state: %d\n",random_state);

        mexPrintf("n_iter_early_exag: %d\n",n_iter_early_exag);
        mexPrintf("early_exaggeration: %f\n",early_exaggeration);
        mexPrintf("learning_rate: %f\n",learning_rate);

        mexPrintf("distance: %d\n",distance);
        mexPrintf("doNormalize: %d\n",doNormalize);

    }
    
    if (distance == 0) {
        TSNE<SplitTree, euclidean_distance> tsne;
        tsne.run(X, N, D, Y, no_dims, perplexity, theta, num_threads, max_iter, n_iter_early_exag, random_state,
             init_from_Y, verbose, early_exaggeration, learning_rate, final_error,doNormalize);
    }   
    else if (distance == 1) {
        TSNE<SplitTree, euclidean_distance_squared> tsne;
        tsne.run(X, N, D, Y, no_dims, perplexity, theta, num_threads, max_iter, n_iter_early_exag, random_state,
