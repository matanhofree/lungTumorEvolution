...
void TSNE<treeT, dist_fn>::run_step2(int* row_P, int* col_P, double* val_P,int N, double* Y,
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    int* row_P = (int *)((reinterpret_cast<double*>(mxGetData(prhs[0]))));
    int* col_P = (int *)((reinterpret_cast<double*>(mxGetData(prhs[1]))));
    double* val_P = mxGetPr(prhs[2]);
    
    int N = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[3]))));
    
    int no_dims = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[4]))));
    double perplexity = *(mxGetPr(prhs[5]));
    double theta = *(mxGetPr(prhs[6]));
    int num_threads = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[7]))));
    int max_iter = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[8]))));
    
    int random_state = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[9]))));
    int verbose = (int)(*(reinterpret_cast<double*>(mxGetData(prhs[10]))));

    double early_exaggeration = *(mxGetPr(prhs[11]));
    double learning_rate = *(mxGetPr(prhs[12]));
...
#define DEBUG_P
#ifdef DEBUG_P
    mexPrintf("N by N: %d x %d\n",N,N);
    mexPrintf("no_dims: %d\n",no_dims);
    mexPrintf("perplexity: %f\n",perplexity);
    mexPrintf("theta: %f\n",theta);
    mexPrintf("num_threads: %f\n",num_threads);
    mexPrintf("maxiter: %d\n",max_iter);
#endif
    ...
    TSNE<SplitTree, euclidean_distance_squared> tsne;
    tsne.run_step2(row_P,col_P,val_P, N, Y, no_dims, perplexity, theta, num_threads, max_iter, random_state,
