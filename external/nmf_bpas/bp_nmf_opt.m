...
function [W,H,iter,HIS,final]=bp_nmf_opt(A,k,W_init,H_init,inOpts)
...
%     end
    ...
    HIS = 0;
    if opts.verbose          % collect information for analysis/debugging
...
%------------------------------------------------------------------------------------------------------------------------------
        ...
        if (iter > opts.min_iter)
            ...
            if (opts.verbose && (tTotal > opts.max_time)) || (~opts.verbose && ((cputime-tStart)>opts.max_time))
                break;
            end
