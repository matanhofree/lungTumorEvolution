...
function [ X,Y,iter,success,pinvCounter ] = nnlsm_blockpivot( A, B, isInputProd, init )
    ...
    if nargin<4
        ...
        PassiveSet = (init > 0);
        [ X,iter,pinvCounter ] = solveNormalEqComb(AtA,AtB,PassiveSet);
        Y = AtA * X - AtB;
    end
    % parameters
    pbar = 3;
    ...
    while(~isempty(find(NotOptCols,1)))
        ...
        [ X(:,NotOptCols),subiter,pinvCounter ] = solveNormalEqComb(AtA,AtB(:,NotOptCols),PassiveSet(:,NotOptCols));
        iter = iter + subiter;
